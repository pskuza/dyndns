...
class RSSTest extends TestCase
{
    ...
    public function testRegister() {
        ...
        shell_exec('sed -i "s/allow_register = true/allow_register = false/g" /tmp/dyndns/config.ini');
        $r = $this->client->request('GET', '/register');
        $this->assertEquals(403, $r->getStatusCode());

        //register set to true and token set
        shell_exec('sed -i "s/allow_register = false/allow_register = true/g" /tmp/dyndns/config.ini');
        shell_exec('sed -i "s/register_token =/register_token ='.bin2hex(random_bytes(32)).'/g" /tmp/dyndns/config.ini');
        $r = $this->client->request('GET', '/register');
        $this->assertEquals(200, $r->getStatusCode());
    }
}